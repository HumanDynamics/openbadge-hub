...
angular.module('ngOpenBadge.services').factory('OBSBluetooth', function($cordovaBluetoothLE, $q, $struct, $timeout, $interval, OBSMyProject) {
  ...
  BluetoothFactory.startScan = function() {
    ...
    $cordovaBluetoothLE.startScan({services: []}).then(
      null,
      function startscan_error(obj) {
        console.log("Starting Scan Error");
        if (CRITICAL_LOGGING) {
        ...
        if (obj.status === "scanResult") {
          ...
          if (obj.name === "HDBDG") {
            ...
            if (obj.address in badgeMap) {
              ...
              // TODO MAKE CONSTANTS
              if (adBadgeData.voltage > 2.8) {
                adBadgeData.battery = 'ion-battery-full';
              } else if (adBadgeData.voltage > 2.5) {
  ...
  BluetoothFactory.discoverAndSubscribe = function(badge) {
    ...
    var subscribeToDevice = function(successObj) {
      ...
      $cordovaBluetoothLE.subscribe(paramsObj).then(
        null,
  ...
  BluetoothFactory.connect = function(badge) {
    ...
    $cordovaBluetoothLE.connect({address: address}).then(
      ...
      function(error) {
        ...
        // we failed to connect - d/c and recursively try again
        return BluetoothFactory.endConnection(badge).then(function () {
          return BluetoothFactory.connect(badge);
        });
      },
  ...
  BluetoothFactory.maintain = function(badge, defered) {
    ...
    var connectTimeout = $timeout(function() {
      console.log("The maintenance to", badge.address, "has timedout");
      BluetoothFactory.endConnection(badge);
