...
angular.module('ngOpenBadge.services').factory('OBSAnalysis', function() {
  var Analysis = {};
  ...
  var checkIntersect = function(startA, endA, startB, endB) {
   ...
   */
  Analysis.DataAnalyzer = function() {
    ...
    this.addSample = function(vol, timestamp, duration) {
      ...
      var isSpeak = volPower > NOISE_POWER_THRESHOLD
        ? 1
        : 0;
  ...
  *******************************************************/
  ...
  var generateTalkIntervals = function(speakSamples) {
    ...
    while (i < speakSamples.length) {
      ...
      while (j < speakSamples.length - 1 && isWithinTalkTimeout(speakSamples[j].timestamp, speakSamples[j + 1].timestamp)) {
        j = j + 1
      }

      // new interval ended. Because each sample is duration millisecond long, we add this to the endTime
      var newTalkInterval = {
        'startTime': speakSamples[i].timestamp,
        'endTime': speakSamples[j].timestamp + speakSamples[j].duration
      };
      ...
      if (isMinTalkLength(newTalkInterval)) {
  ...
  var FixedLengthArray = function(numSamples) {
  ...
  var SmoothArray = function(numSamples) {
    var numSamples = numSamples
    // Number of samples that will be used for smoothing the samples
    var smoothArray = [];
    var pos = 0;
    ...
    this.push = function(vol) {
      ...
      var sum = smoothArray.reduce(function(a, b) {
        return a + b;
      });
  ...
  var MedianArray = function(numSamples) {
    var numSamples = numSamples
    // Number of samples that will be used for smoothing the samples
    var samplesArray = [];
    var pos = 0;
  ...
  var MinArray = function(numSamples) {
    var numSamples = numSamples
  ...
  // P = 1/N*sum((x-mean)^2)
  var SlidingPower = function(numSamples) {
    var numSamples = numSamples
    // Number of samples that will be used for smoothing the samples
    var samplesArray = [];
    var pos = 0;
  ...
  var average = function(data, getValue) {
  ...
  var meanAndStd = function(data, getValue) {
    // calc mean
    ...
    var avgSquareDiff = average(squareDiffs, function(v) {
      return v
    });
   ...
   */
  Analysis.GroupDataAnalyzer = function(members, periodStartTime, periodEndTime) {
    ...
    for (var d = periodStartTime; d <= periodEndTime; d = d + INTERVAL_INCREMENTS) {
      ...
      $.each(members, function(index, member) {
        ...
        if (pos == -1)
          return; // already reached end of array, move to the next member
  ...
  var filterPeriod = function(samples, start, end) {
