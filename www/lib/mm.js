...
(function() {
  ...
  this.MM = (function() {
    function MM(data, localParticipant, width, height) {
      ...
      this.constantRotationAngle = bind(this.constantRotationAngle, this);
      ...
      this.sphereColorScale = d3.scale.linear().domain([0, data.participants.length * 3]).range(['#C8E6C9', '#2E7D32']).clamp(true);
      this.nodes = (function() {
    ...
    MM.prototype.renderNodes = function() {
      ...
      this.nodeG.append("text").attr("text-anchor", "middle").attr("font-size", "24px").attr("dy", ".35em").attr("transform", (function(_this) {
        return function(d) {
          if (d.participant === 'energy') {
            return "";
          } else {
            return "rotate(" + (-1 * (_this.constantRotationAngle() + _this.angle(d.participant))) + ")";
          }
        };
      })(this)).attr("fill", (function(_this) {
        return function(d) {
          if (d.participant === _this.localParticipant) {
            return "#FFFFFF";
          } else {
            return "#000000";
          }
    ...
    MM.prototype.createLinks = function() {
      var j, len, participant, ref, results, turn;
      this.links = (function() {
        var j, len, ref, results;
        ref = this.data.turns;
        results = [];
    ...
    MM.prototype.constantRotationAngle = function() {
      var a, angle, mod, targetAngle;
      mod = function(a, n) {
        return a - Math.floor(a / n) * n;
      };
      angle = this.angle(this.localParticipant) || 0;
      targetAngle = -90;
      a = targetAngle - angle;
      a = (a + 180) % 360 - 180;
      if (angle !== -90) {
